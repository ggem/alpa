...
(define cpu-time		;;; returns a pair (program . program+gc)
  (lambda ()
    (let ([stt (statistics)])
	 ...
	 (let ([start (cpu-time)])
	   ...
	   (let ([middle (cpu-time)])
	     ...
	     (let ([end (cpu-time)])
	       (let ([five (- end middle)]
		     [four (- middle start)])
		 (let ([time (/ (- five four) 1.0 iterations)])
		   (printf "~s: ~s milliseconds (~s)~n" 'exp time (- five four))
		   time)))))))])))
...
'(do-tests 20
  (2500 (quicksort iota-0050))
  ( 640 (quicksort iota-0100))
  ( 160 (quicksort iota-0200))
  (  64 (quicksort iota-0300))
  (  16 (quicksort iota-0500))
  (   4 (quicksort iota-1000))
  (   1 (quicksort iota-2000)))



'(optimize-level 3)
'(load "costbound-nreverse.ss")
