...
(define-syntax weird-max
  (lambda (x)
    (syntax-case x ()
      [(_ level exp1 exp2)
       (syntax
	 (cond
	   [(negative? current-level) (max exp1 exp2)]
	   [(zero? (fxlogand current-level (fxsll 1 level))) exp1]
	   [else exp2]))])))
...
(define-syntax weird-max
  (lambda (x)
    (syntax-case x ()
      [(_ level exp1 exp2)
       (if (> (weight (syntax-object->datum (syntax exp1)))
	     (weight (syntax-object->datum (syntax exp2))))
	 (syntax exp1)
	 (syntax exp2))])))
