  ...
  (syncase obj
    ...
    [`(marker var ,var ,val)
     (syncase val
       [`(marker pair ,the-car ,the-cdr) the-car]
       [else `(car ,obj)])]
    [`(marker ,x . ,y) (error 'prim:car "not a pair: ~a" obj)]
    [else `(car ,obj)]))
  ...
  (syncase obj
    ...
    [`(marker var ,var ,val)
     (syncase val
       [`(marker pair ,the-car ,the-cdr) the-cdr]
       [else `(cdr ,obj)])]
    [`(marker ,x . ,y) (error 'prm:cdr "not a pair: ~a" obj)]
    [else `(cdr ,obj)]))
  ...
  (syncase obj
    ...
    [`(marker var ,var ,val)
     (syncase val
       ...
       [`(marker pair . ,ignored) `(marker boolean #f)]
       [else `(null ,obj)])]
    [`(marker . ,ignored) (error 'prim:null? "not a list: ~a" obj)]
    [else `(null? ,obj)]))

(define (prim:unknown? obj)
  (syncase obj
    [`(marker var ,var (marker . ,ignored)) '(marker boolean #f)]
    [`(marker var . ,ignored) `(unknown? ,obj)]
    [`(marker . ,ignored) '(marker boolean #f)]
    [else `(unknown? ,obj)]))
...
(define (prim:< obj1 obj2)
  (syncase obj1
	...
	(if (< x y)
	  `(marker boolean #t)
	  `(< ,obj1 ,obj2))]
       [`(marker var ,var (+ (marker number ,y) . ,z))
	(if (< x y)
	  `(marker boolean #t)
	  `(< ,obj1 ,obj2))]
       [else `(< ,obj1 ,obj2)])]
	...
	(if (< y x)
	  ...
	  `(< ,obj1 ,obj2))]
       [else `(< ,obj1 ,obj2)])]
    [`(marker var ,var (+ (marker number ,x) . ,z))
     (syncase obj2
       [`(marker number ,y)
	(if (< y x)
	  `(marker bookean #f)
...
(define (prim:list . args)
  (do ([args (reverse args) (cdr args)]
       [answ '(marker null '()) (prim:cons (car args) answ)])
    [(null? args) answ]))
...
(define (make-initial-environment)
  `((car . ,prim:car)
    ...
    (T- . (marker number ,T-))
    (T* . (marker number ,T*))
    (Tquotient . (marker number ,Tquotient))
    (T> . (marker number ,T>))
    (T< . (marker number ,T<))
    (T= . (marker number ,T=))
    (Tc . (marker number ,Tc))
  ...
  (syncase exp
    ...
    [`(if ,test-exp ,then-exp ,else-exp)
	 ...
	 (syncase exps
	   ...
	   [`((marker var . ,ignored) . ,ignored2)
	    (k (cons 'if exps) vars-used)]
	   [`((marker . ,ignored) ,then-exp ,else-exp) (k then-exp vars-used)]
	   [`(,test-exp ,then-exp ,else-exp)
	    (if (equal? then-exp else-exp)
	      (k then-exp vars-used)
	      (k (cons 'if exps) vars-used))])))]
    [`(,proc . ,args)
     (map-eval exp env
       (lambda (exps vars-used)
...
(define (marked->unmarked exp)
  (syncase exp
    ...
    [`(marker var ,var ,val) var]
    [`(marker symbol ,val) `',val]
...
(define (optimize x)
  (let ([env (make-initial-environment)]
	[init-k (lambda (exp vars-used) exp)])
    (do ([x x (marked->unmarked (eval x env init-k))]
	 [old-x #f x])
      [(equal? old-x x) x])))
